...
LUA_API int luaopen_pb_libs(lua_State* L) {
    luaL_newmetatable(L, "__pb__table");
    lua_pushvalue(L, -1);
...
{
    luaL_requiref(L, "pb", luaopen_pb_libs, 0);
    luaL_newmetatable(L, IOSTRING_META);
    lua_pushvalue(L, -1);
    lua_setfield(L, -2, "__index");
    luaL_setfuncs(L, _c_iostring_m, 0);
    lua_setglobal(L, IOSTRING_META);
